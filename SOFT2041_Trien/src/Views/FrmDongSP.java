...
public class FrmDongSP extends javax.swing.JFrame {
    ...
    private List<QLDongSP> listQlDongSP;
    private DefaultTableModel dtm;
    private QLDongSPServiceImpl qldspsi;
    ...
    public FrmDongSP() {
        initComponents();
        listQlDongSP = new ArrayList<>();
        qldspsi = new QLDongSPServiceImpl();
        jTable1.setModel(dtm = new DefaultTableModel());
        String[] header = {"ID", "Ma", "Ten"};
        dtm.setColumnIdentifiers(header);
        listQlDongSP = qldspsi.getAll();
    ...
    private void initComponents() {
        ...
        jLabel1.setText("Quan Ly Dong SP");
    ...
    private void showData(List<QLDongSP> listDongSP) {
        dtm.setRowCount(0);
        for (QLDongSP qLDongSP : listDongSP) {
            dtm.addRow(qLDongSP.toDataRow());
        }
    }

    private void fillData(int index) {
        QLDongSP p = listQlDongSP.get(index);
    ...
    private void insert() {
        ...
        if (ma.length() == 0 || ten.length() == 0) {
            ...
            QLDongSP qLDongSP = new QLDongSP(ma, ten);
            JOptionPane.showMessageDialog(this, qldspsi.insert(qLDongSP));
            showData(listQlDongSP = qldspsi.getAll());
        }
    }
    ...
    private void update() {
        int row = jTable1.getSelectedRow();
        if (row >= 0) {
            QLDongSP ql = listQlDongSP.get(row);
            UUID id = ql.getId();
            String ma = txtMa.getText().trim();
            String ten = txtTen.getText().trim();
            if (ma.length() == 0 || ten.length() == 0) {
                ...
                QLDongSP qLDongSP = new QLDongSP(ma, ten);
                JOptionPane.showMessageDialog(this, qldspsi.update(qLDongSP, id));
                showData(listQlDongSP = qldspsi.getAll());
            }
        } else {
    ...
    private void delete() {
        ...
        if (row >= 0) {
            QLDongSP ql = listQlDongSP.get(row);
            UUID id = ql.getId();
            JOptionPane.showMessageDialog(this, qldspsi.delete(id));
            showData(listQlDongSP = qldspsi.getAll());
        } else {
            JOptionPane.showMessageDialog(this, "Chon doi tuong de sua");
        }
